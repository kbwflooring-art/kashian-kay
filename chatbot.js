(function () {
  ...
  style.innerHTML = [
    ...
    '#kb-chat-btn .kb-avatar{width:36px;height:36px;display:flex;align-items:center;justify-content:center;flex-shrink:0;}',
    ...
    '.kb-time-btn{font-family:inherit;cursor:pointer;background:#fff;border:1.5px solid #5bcdc7;color:#0e7490;border-radius:8px;padding:9px 6px;font-size:11.5px;font-weight:500;text-align:center;line-height:1.3;min-width:0;word-break:normal;overflow-wrap:break-word;}',
  ...
  btn.id = 'kb-chat-btn';
  btn.innerHTML = '<span class="kb-avatar"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 640 512" fill="#fff"><path d="M208 352c114.9 0 208-78.8 208-176S322.9 0 208 0S0 78.8 0 176c0 38.6 14.7 74.3 39.6 103.4c-3.5 9.4-8.7 17.7-14.2 24.7c-4.8 6.2-9.7 11-13.3 14.3c-1.8 1.6-3.3 2.9-4.3 3.7c-.5 .4-.9 .7-1.1 .8l-.2 .2 0 0 0 0C1 327.2-1.4 334.4 .8 340.9S9.1 352 16 352c21.8 0 43.8-5.6 62.1-12.5c9.2-3.5 17.8-7.4 25.3-11.4C134.1 343.3 169.8 352 208 352zM448 176c0 112.3-99.1 196.9-216.5 207C255.8 457.4 336.4 512 432 512c38.2 0 73.9-8.7 104.7-23.9c7.5 4 16 7.9 25.2 11.4c18.3 6.9 40.3 12.5 62.1 12.5c6.9 0 13.1-4.5 15.2-11.1c2.1-6.6-.2-13.8-5.8-17.9l0 0 0 0-.2-.2c-.2-.2-.6-.4-1.1-.8c-1-.8-2.5-2-4.3-3.7c-3.6-3.3-8.5-8.1-13.3-14.3c-5.5-7-10.7-15.4-14.2-24.7c24.9-29 39.6-64.7 39.6-103.4c0-92.8-84.9-168.9-192.6-175.5c.4 5.1 .6 10.3 .6 15.5z"/></svg></span><span>Questions? Scheduling? Let\'s chat</span>';
  ...
  document.getElementById('kb-close-btn').onclick = function () { widget.style.display = 'none'; sendChatLog(); };

  // ----- CONVERSATION LOG EMAIL -----
  // Send transcript to dstein@kashianbros.com when customer closes chat or leaves page
  // Post an abandoned Contact Us form to the Google Form -> Kay Leads sheet, tagged
  // "Abandoned - Contact Form" so it's easy to tell apart from submitted leads.
  // Fire-and-forget (beacon) since it runs as the customer is leaving the page.
  function postAbandonedLead(f) {
    try {
      if (!LEADS_FORM_URL) return;
      var params = [];
      function add(entry, val) { if (val) params.push(entry + '=' + encodeURIComponent(val)); }
      add(LEADS_FIELDS.date, new Date().toLocaleString('en-US', { timeZone: 'America/Chicago' }));
      ...
      try {
        if (navigator.sendBeacon) {
          ...
          if (navigator.sendBeacon(LEADS_FORM_URL, blob)) return;
        }
      } catch (e) { /* fall through to fetch */ }
      fetch(LEADS_FORM_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: body, keepalive: true }).catch(function () {});
    } catch (e) {}
  }
  ...
  window.addEventListener('pagehide', sendChatLog);
  ...
  // =========================
  // ESTIMATE / CALLBACK FORM
  // =========================
  // Standalone lead-capture form for services we do NOT book online (hardwood,
  // remodels, custom carpet/rugs, etc.) and for any "please contact me" request.
  // Shown either from the menu button or when Kay ends a reply with [SHOW_ESTIMATE_FORM].
  function buildEstimateForm(mid) {
    ...
    f.innerHTML =
      ...
      '<div class="kb-row-2">' +
        ...
        '<div class="kb-frow"><label class="kb-label">Last Name *</label><input class="kb-input" id="kb-eln-' + mid + '" placeholder="Smith"/></div>' +
      '</div>' +
      '<div class="kb-row-2">' +
        '<div class="kb-frow"><label class="kb-label">Phone *</label><input class="kb-input" id="kb-eph-' + mid + '" placeholder="(847) 555-1234" type="tel"/></div>' +
        '<div class="kb-frow"><label class="kb-label">Email *</label><input class="kb-input" id="kb-eem-' + mid + '" placeholder="you@email.com" type="email"/></div>' +
      '</div>' +
      '<div class="kb-frow"><label class="kb-label">Address (optional)</label><input class="kb-input" id="kb-ead-' + mid + '" placeholder="123 Main St, Wilmette, IL 60091" data-addr="1" autocomplete="off"/></div>' +
      '<div class="kb-frow"><label class="kb-label">Project Details *</label><textarea class="kb-textarea" id="kb-edt-' + mid + '" rows="3" placeholder="Tell us what you are looking to do..."></textarea></div>' +
      '<button class="kb-submit" onclick="kbSubmitEstimate(\'' + mid + '\')">Send Request &rarr;</button>' +
      '<button class="kb-cancel" onclick="kbCancelEstimate(\'' + mid + '\')">Never mind, I will call instead</button>';
    return f;
  }
  window.kbCancelEstimate = function (mid) { var f = document.getElementById('kb-est-' + mid); if (f) f.innerHTML = '<p style="color:#333;font-size:12.5px">No problem! Call us at <strong>(847) 251-1200</strong> or email <strong>info@kashianbros.com</strong>!</p>'; flow.active = false; };
  window.kbSubmitEstimate = function (mid) {
    var fn = v('kb-efn-' + mid), ln = v('kb-eln-' + mid), ph = v('kb-eph-' + mid), em = v('kb-eem-' + mid), ad = v('kb-ead-' + mid), dt = v('kb-edt-' + mid);
    if (!fn || !ln || !ph || !em || !dt) { alert('Please fill in your name, phone, email, and project details.'); return; }
    var name = (fn + ' ' + ln).trim();
    var isChicago = /\bchicago\b/i.test(ad);
    var eb = 'ESTIMATE REQUEST - KASHIAN BROS KAY\nName: ' + name + '\nPhone: ' + ph + '\nEmail: ' + em + (ad ? '\nAddress: ' + ad : '') + '\nProject: ' + dt + '\nSubmitted: ' + new Date().toLocaleString();
    fetch(BOOKING_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name, phone: ph, email: em, addr: ad, svc: 'ESTIMATE', lbl: 'Estimate Request', detail: dt, stairs: '', date: '', time: '', selfP: true, cname: '', cphone: '', pets: '', notes: dt, isChicago: isChicago, truck: '', emailBody: eb }) }).catch(function () {});
    // Insurance: also drop the lead into the transcript so it appears in the chat-log
    // email to Doug even if the booking function ever fails to send.
    hist.push({ role: 'user', content: eb });
    lastBookingSummary = eb;
    var card = document.getElementById('kb-est-' + mid);
    if (card) {
      card.className = 'kb-success';
      card.innerHTML = '<h3>Request Sent! \u2713</h3><p><strong>Thank you, ' + fn + '!</strong></p><p>We have sent your info to our team and someone will contact you soon.</p><p>Need anything sooner? Call <strong>(847) 251-1200</strong>.</p>';
      scrollNice(card);
    }
    flow.active = false;
  };
  ...
  // =========================
  // RUG PICKUP WINDOWS BY CITY
  // =========================
  // Rug pickup/delivery runs a Tue/Thu route; each city has a FIXED arrival window.
  // The customer picks the DAY; the window is set by their city. Cities not listed
  // (and Chicago, which we can't reliably split north/south of Irving Park from a
  // typed address) fall through to a manual confirm - the request still goes to
  // Adolfo, who calls the customer to set a window.
  var PICKUP_WINDOWS = {
    'skokie': '8:00-10:00 AM', 'evanston': '8:00-10:00 AM', 'lincolnwood': '8:00-10:00 AM', 'niles': '8:00-10:00 AM',
    'wilmette': '9:00-11:00 AM', 'kenilworth': '9:00-11:00 AM',
    'winnetka': '10:00 AM-12:00 PM', 'northfield': '10:00 AM-12:00 PM', 'glencoe': '10:00 AM-12:00 PM',
    'highland park': '11:00 AM-1:00 PM', 'northbrook': '11:00 AM-1:00 PM', 'deerfield': '11:00 AM-1:00 PM', 'riverwoods': '11:00 AM-1:00 PM',
    'lake forest': '12:00-2:00 PM', 'lake bluff': '12:00-2:00 PM', 'highwood': '12:00-2:00 PM',
    'glenview': '1:00-3:00 PM', 'golf': '1:00-3:00 PM', 'morton grove': '1:00-3:00 PM'
  };
  // Pull the city out of an address by matching WHOLE comma-segments (not substrings),
  // so street names like "Golf Rd" or "Niles Center Rd" can't false-match a city.
  function lookupPickupWindow(addr) {
    if (!addr) return null;
    var segs = String(addr).toLowerCase().split(',');
    for (var i = 0; i < segs.length; i++) {
      var seg = segs[i].replace(/\b(il|illinois|usa|us)\b/g, '').replace(/[0-9]/g, '').replace(/\s+/g, ' ').trim();
      if (PICKUP_WINDOWS[seg]) return PICKUP_WINDOWS[seg];
    }
    return null;
  }
  // Next N upcoming Tuesdays/Thursdays (never today), formatted like "Tuesday, March 5".
  function nextRugDates(n) {
    var out = [];
    var cs = new Date().toLocaleDateString('en-US', { timeZone: 'America/Chicago', year: 'numeric', month: '2-digit', day: '2-digit' });
    var p = cs.split('/'); var mo = parseInt(p[0], 10) - 1, da = parseInt(p[1], 10), yr = parseInt(p[2], 10);
    for (var i = 1; out.length < n && i < 30; i++) {
      var d = new Date(yr, mo, da + i);
      var dow = d.getDay();
      if (dow === 2 || dow === 4) out.push(d.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }));
    }
    return out;
  }
  ...
  function renderRugWindows(pfx, mid) {
    ...
    flow.rugManual = false;
    var win = lookupPickupWindow(flow.customerInfo && flow.customerInfo.addr);
    if (win) {
      var dates = nextRugDates(4);
      var html = '<div class="kb-stage-hdr">Choose your pickup day</div>';
      html += '<div style="font-size:12px;color:#475569;margin-bottom:8px;line-height:1.5">Our truck comes to your area between <strong>' + win + '</strong> on Tuesdays and Thursdays. Pick the day that works best:</div>';
      ...
      flow.rugManual = true;
      var h2 = '<div class="kb-stage-hdr">Pickup window</div>';
      h2 += '<div style="font-size:12.5px;color:#475569;padding:4px 0 10px;line-height:1.55">Your area isn\'t on our standard Tuesday/Thursday route, so <strong>Adolfo will call you to confirm a pickup window</strong>. Go ahead and submit your request and we\'ll be in touch soon.</div>';
      h2 += '<button class="kb-submit" onclick="kbPickTime(\'' + mid + '\',\'Pickup window to be confirmed by phone\',\'' + pfx + '\')">Continue with request &rarr;</button>';
      s2.innerHTML = h2;
    }
  }
  ...
  window.kbFinalBooking = function (mid) {
    ...
    var manualNote = (sc.isRug && flow.rugManual) ? '*** MANUAL PICKUP WINDOW - CALL CUSTOMER TO CONFIRM ***\n' : '';
  ...
  function addMsg(role, content, id) {
    ...
    var showEstimate = content.indexOf('[SHOW_ESTIMATE_FORM]') !== -1;
    var infoMatch = content.match(/\[SHOW_INFO_FORM:(CARPET|RUG)\]/);
    var clean = content.replace(/\[SHOW_SCOPE\]/g, '').replace(/\[SHOW_RUG_SCOPE\]/g, '').replace(/\[SHOW_ESTIMATE_FORM\]/g, '').replace(/\[SHOW_INFO_FORM:(CARPET|RUG)\]/g, '').trim();
    ...
    if (showEstimate && isBot) wrap.appendChild(buildEstimateForm(id));
    ...
    sections.forEach(function (group) {
      ...
      group.b.forEach(function (b) {
        ...
        btn.onclick = function () {
          ...
          else if (b.a === 'estimate') startEstimateFlow();
  ...
  function kbSend(text) {
    ...
    addMsg('user', text, cnt); hist.push({ role: 'user', content: text });
    addTyping();
    fetch(CHAT_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 800, temperature: 0, system: SP + (availCache ? '\n\n' + availCache : ''), messages: hist }) })
